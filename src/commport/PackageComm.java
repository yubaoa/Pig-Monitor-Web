...
public class PackageComm implements SerialPortEventListener,Runnable{
   ...
   // private int numBytes; 
    ...
    public PigHouse pighouse;
    public static void main(String[] args){
 
    	PackageComm packageComm = new PackageComm();
    	packageComm.start();
    	//packageComm.sendToComm(new Pig(1429210036,1,1,1,1,1,1,1));
    }
	...
	/*public void sendToComm(Restaurantinf restaurantinf){
		byte[] sendInfo = new byte[21];
		sendInfo[0] = start_1;
		sendInfo[1] = start_2;
		sendInfo[2] = 0x00;
		sendInfo[3] = 0x00;
		if(restaurantinf.getSensorid().equals("1429210036")){
			sendInfo[4]=0x01;
			sendInfo[5]=0x00;
			sendInfo[6]=0x00;
			sendInfo[7]=0x00;
		...
		if (restaurantinf.getLight1statu()==1) {
			sendInfo[8] = 0x01;
			sendInfo[9] = 0x01;
		}else{
			sendInfo[8] = 0x01;
		...
		if (restaurantinf.getLight2statu()==1) {
			sendInfo[10] = 0x02;
			sendInfo[11] = 0x01;
		}else{
			sendInfo[10] = 0x02;
		...
		if (restaurantinf.getLight3statu()==1) {
			sendInfo[12] = 0x03;
			sendInfo[13] = 0x01;
		}else{
			sendInfo[12] = 0x03;
		...
		if (restaurantinf.getLight4statu()==1) {
			sendInfo[14] = 0x04;
			sendInfo[15] = 0x01;
		}else{
			sendInfo[14] = 0x04;
 ...
 */
	        ...
	        case SerialPortEvent.DATA_AVAILABLE:
	        	...
	        	try{
	        		...
	        		while(inputStream.available()>0){
	        			...
	        			if (str[0] == 0x0A) {
							pig_bytesToDatas(str);
							if (validateflag)
								pig_house_bytesToDatas(str);
						}else {
							//new PaySuccessResponseThread(NecessaryInformation.socket).run();
						}
	        			if (pighouse != null && pig != null) {//得到的数据有效
							PigDao pigdao =new PigDao();
							pigdao.updatapiginf(pig);
							PighouseDao pighousedao=new PighouseDao();
							pighousedao.updatapighouseinf(pighouse);
	        			}
	        		}
	        	}catch(IOException e){
	        		e.printStackTrace();
	...
	private Pig pig_bytesToDatas(byte[] data){
		pig = null;
		validateflag=false;
		//if(data[0]==start_1 && data[1] == start_2 && data[40] ==end_1&& data[41] ==end_2&& data[42] ==end_3)
		if(data[0]==start_1 && data[1] == start_2 && data[38] ==end_1&& data[39] ==end_2&& data[40] ==end_3)
		{
			...
			if(length_i == 32){
				pig = new Pig();
				pighouse = new PigHouse();
				busId[0] = data[4];
				busId[1] = data[5];
				busId[2] = data[6];
				busId[3] = data[7];
				if(bytesToInt(busId)==1){
					sensor_id_inner = "123456789";
				}
				busStatus = data[8];
				busType = data[9];
				...
				crc[2] = data[36];
				crc[3] = data[37];
				System.out.println(bytesToShort(busTemperate));
				int temp=bytesToShort(busTemperate);
				pig.setBodytemperature((double) temp);//将从串口得到的数据处理后传给实体类
				pig.setRecorddate(new Date().toString());
				//pighouse.setTemperature();//待写
				//System.out.println(restaurantinf.toString());
				/*longitude[0] = data[14];
				longitude[1] = data[15];
				longitude[2] = data[16];
				longitude[3] = data[17];
				...
				crc[2] = data[38];
				crc[3] = data[39];*/
				/*				restaurantinf.setSensorid(sensor_id_inner);
				restaurantinf.setLight1statu(oneByteToInt(light_one_status));
				System.out.println(restaurantinf.getLight1statu());
				restaurantinf.setLight2statu(oneByteToInt(light_two_status));
				restaurantinf.setLight3statu(oneByteToInt(light_three_status));
				restaurantinf.setLight4statu(oneByteToInt(light_four_status));*/
			}else{
	...
	private PigHouse pig_house_bytesToDatas(byte[] data){
		pighouse = null;
		double k=1600.0/bytesToShort(lightStrength);
		int l= (int) (k*50);
		pighouse.setIllumination(l);//将从串口得到的数据处理后传给实体类
		pighouse.setSmog(bytesToShort(smoke));
		pighouse.setRecordtime(new Date().toString());
		pighouse.setTemperature(bytesToShort(busTemperate));
		return pighouse;
	}
